 ...
 * Utilities.h                       // Title function and ProcessCmdLine class
 *
 * Maintenance History:
 * --------------------
 * ver 1.5 : 03 Feb 2019
 * - Added a vector to store files matching a given pattern. Added
 *	 a method to access that vector.
 * ver 1.4 : 24 Jan 2019
...
namespace FileSystem
{
  ...
  public:
	...
	using processFiles = std::vector<std::string>;
	...
	processFiles getVector();

  private:
    std::string path_;
    patterns patterns_;
	processFiles filesToProcess_;
    size_t dirCount_ = 0;
    size_t fileCount_ = 0;
    bool recurse_ = false;
  };
  ...
  */
  ...
  inline DirExplorerN::processFiles DirExplorerN::getVector()
  {
